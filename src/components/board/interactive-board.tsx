...
export function InteractiveBoard({ orders, onOrderUpdate, updatingOrders = new Set() }: InteractiveBoardProps) {
  ...
  const ordersByStatus = orders.reduce((acc, order) => {
    ...
    console.log(`🎯 Grouping order ${order.order_number}: status="${order.status}" -> mapped="${status}"`)
    if (!acc[status]) {
      acc[status] = []
    }
  ...
  console.log('🎯 InteractiveBoard: Status counts:', Object.keys(ordersByStatus).map(status => `${status}: ${ordersByStatus[status].length}`))
  console.log('🎯 InteractiveBoard: Available columns:', COLUMNS.map(c => c.id))
  ...
  function handleDragEnd(event: DragEndEvent) {
    ...
    console.log('🔄 Drag end:', { activeId: active.id, overId: over?.id, over })
    setActiveOrder(null)
    ...
    if (mappedStatus && onOrderUpdate) {
      ...
      console.log('❌ No update needed:', { orderId, newStatus, mappedStatus, hasOnOrderUpdate: !!onOrderUpdate })
    }
  }
                ...
                <div className="p-2 sm:p-3">
                  <div className="flex justify-between items-start mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <h4 className="font-semibold text-sm sm:text-base">#{activeOrder.order_number}</h4>
                    </div>
                    <RushIndicator
                      isRush={activeOrder.rush || false}
                      orderType={activeOrder.type || 'alteration'}
                  ...
                  <p className="text-xs text-gray-500 mb-1">
                    {activeOrder.garments?.map((g: any) => g.type).join(', ') || 'No garments'}
                  </p>
